    ...
    for event in events:
        ...
        if key not in consolidated:
            consolidated[key] = {
                ...
                "Type": event.Type if event.Type else "",  # Manejar el caso de None
                "User": event.User,
        ...
        if servidor_lookup_id:
            ...
            if chequeo_servidor_id:
                if events:
                    ...
                    error_events = sum(
                        1 for event in consolidated_events 
                        if event["Type"] and event["Type"].lower() == "error"
                    )
                    critical_events = sum(
                        1 for event in consolidated_events 
                        if event["Type"] and event["Type"].lower() == "critical"
                    )
